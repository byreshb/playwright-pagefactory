...
2. **Set the version.** In `pom.xml` change `<version>` to `X.Y.Z` (no `-SNAPSHOT`), and update
   the version in the README's dependency snippets.
...
5. **Verify.** The Release workflow appears under Actions within a minute and finishes in about
   ...
   release is at `https://github.com/byreshb/playwright-pagefactory/releases/tag/vX.Y.Z` with
   three assets attached: the main jar, `-sources.jar` and `-javadoc.jar`.
6. **Start the next cycle.** Set `pom.xml` back to `X.Y.(Z+1)-SNAPSHOT` and add a fresh
   `## [Unreleased]` heading to the changelog.

Consumers who do not use Maven can download the jar from the release page. Maven and Gradle
users still need `mvn install` from a checkout until the library is on Maven Central (see below).
...
5. **Pom additions.** Central validates that the pom has `name`, `description`, `url`,
   ...
   the case). Add a `developers` and `scm` block, then the signing and publishing plugins, ideally
   inside a `release` profile so ordinary builds do not need GPG:
   ...
   <scm>
     <url>https://github.com/byreshb/playwright-pagefactory</url>
     <connection>scm:git:https://github.com/byreshb/playwright-pagefactory.git</connection>
     <developerConnection>scm:git:git@github.com:byreshb/playwright-pagefactory.git</developerConnection>
     <tag>HEAD</tag>
   </scm>
